...
public:
    ...
    void render() {
        while (mRunning) {
            if (mAudioClient.readyToRender(1024)) {
                std::lock_guard<std::mutex> lock(mMutex);
                ...
                renderCallback(mInputBuffer.data(), mOutputBuffer.data(), 1024);
                mAudioClient.render(mInputBuffer.data(), mOutputBuffer.data(), 1024);
            }
